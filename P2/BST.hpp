...
/*
  void print_structure(){
	 	queue<BSTNode<Data>*> pointers;
	 	BSTNode<Data>* node = this->root;
	 	BSTNode<Data>* edge;
	 	pointers.push(node);
	 	int level = 0;
	 	int numCount = 0;
	 	while(pointers.size() > 0){
	 		...
	 		{
	 			edge = pointers.front();
	 			if(edge->left) pointers.push(edge->left);
	 			if(edge->right) pointers.push(edge->right);
	 			pointers.pop();
	 		}
	 		cout << endl;
	 	}
	 	cout << "Number of int is: " << numCount << endl << endl;
	 } 
*/
